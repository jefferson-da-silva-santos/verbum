  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: tokens.bgPrimary }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 16 }}>
        ...
        {step === 1 && (
          <>
            <Text style={{ fontSize: 20, fontWeight: '700', fontFamily: 'serif', color: tokens.textPrimary }}>
              O que você quer ler?
            </Text>
            {PRESET_PLANS.map(p => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  backgroundColor: preset === p.id ? tokens.actionPrimary : tokens.bgCard,
                  borderRadius: 12, borderWidth: 1,
                  borderColor: preset === p.id ? 'transparent' : tokens.borderLight,
                }}
              >
                <MaterialCommunityIcons
                  name={p.icon as keyof typeof MaterialCommunityIcons.glyphMap}
                  size={26}
                  color={preset === p.id ? tokens.actionPrimaryText : tokens.actionPrimary}
                />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: '600', color: preset === p.id ? tokens.actionPrimaryText : tokens.textPrimary }}>
                    {p.name}
                  </Text>
                  <Text style={{ fontSize: 12, color: preset === p.id ? tokens.actionPrimaryText : tokens.textTertiary, opacity: 0.8 }}>
                    {p.totalChapters} capítulos
                  </Text>
                </View>
            ...
            {(['chapters', 'time', 'deadline'] as const).map(m => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  backgroundColor: mode === m ? tokens.actionPrimary : tokens.bgCard,
                  borderColor: mode === m ? 'transparent' : tokens.borderLight,
                  gap: 4,
                }}
              >
            ...
            {mode === 'deadline' ? (
              <View style={{ gap: 10 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, backgroundColor: tokens.bgCard, borderRadius: 12, padding: 16, borderWidth: 1, borderColor: tokens.borderLight }}>
                  <TouchableOpacity onPress={() => setDeadlineDays(d => Math.max(7, d - 7))}>
                    <MaterialCommunityIcons name="minus-circle-outline" size={28} color={tokens.iconPrimary} />
                  </TouchableOpacity>
                  <View style={{ flex: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: 28, fontWeight: '700', color: tokens.textPrimary }}>{deadlineDays}</Text>
                    <Text style={{ fontSize: 12, color: tokens.textTertiary }}>dias a partir de hoje</Text>
                  </View>
                  <TouchableOpacity onPress={() => setDeadlineDays(d => Math.min(365, d + 7))}>
                ...
                <Text style={{ fontSize: 12, color: tokens.textTertiary, textAlign: 'center' }}>
                  Termina em: <Text style={{ fontWeight: '700', color: tokens.textSecondary }}>{targetDate}</Text>
                </Text>
              </View>
            ) : (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, backgroundColor: tokens.bgCard, borderRadius: 12, padding: 16, borderWidth: 1, borderColor: tokens.borderLight }}>
...
function Row({ label, value }: { label: string; value: string }) {
  ...
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
      ...
      <Text style={{ fontSize: 14, fontWeight: '600', color: tokens.textPrimary }}>{value}</Text>
    </View>
  );
}